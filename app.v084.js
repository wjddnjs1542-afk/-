...
async function recalculateLiveRoute(){
 ...
 try{
   ...
   const renderer=new google.maps.DirectionsRenderer({
     ...
     polylineOptions:{strokeColor:'#ff3b5c',strokeOpacity:.94,strokeWeight:7}
   });
   renderer.setDirections(result);routeRenderers.push(renderer);
   const leg=result.routes[0].legs[0];
   const fallback=routed.fallback?' · 대중교통 경로가 없어 도보로 표시':'';
   ...
   $('#liveRouteStatus').textContent=`${liveModeLabel(routed.actualMode)}${routed.fallback?' 대체':''}`;
   ...
   showApproximateRoute(currentPosition,{lat,lng},name,e);
   const km=hav(currentPosition,{lat,lng});
   const mins=estimateMinutes(currentPosition,{lat,lng},currentMode);
   $('#liveRouteDistance').textContent=km<1?`${Math.round(km*1000)}m`:`${km.toFixed(1)}km`;
   $('#liveRouteDuration').textContent=`약 ${Math.round(mins)}분`;
   $('#liveRouteStatus').textContent='직선 임시 안내';
   toast(e.status==='REQUEST_DENIED'?'Directions API 설정이 필요해 임시 안내를 표시했습니다.':'도로 경로가 없어 임시 안내를 표시했습니다.');
 }finally{
...
function requestRoute(a,b,mode,i){return new Promise((resolve,reject)=>{
 const req={origin:new google.maps.LatLng(Number(a.lat),Number(a.lng)),destination:new google.maps.LatLng(Number(b.lat),Number(b.lng)),travelMode:google.maps.TravelMode[mode],unitSystem:google.maps.UnitSystem.METRIC,provideRouteAlternatives:false};
 if(mode==='TRANSIT')req.transitOptions={departureTime:new Date(Date.now()+5*60*1000),routingPreference:google.maps.TransitRoutePreference.FEWER_TRANSFERS};
...
async function routeSegment(a,b,mode,i){
 const order=[mode,...(['WALKING','DRIVING','TRANSIT'].filter(x=>x!==mode))];let firstError=null;
 for(const candidate of order){
  try{const r=await requestRoute(a,b,candidate,i);return{...r,fallback:candidate!==mode,requestedMode:mode}}catch(e){firstError=firstError||e;if(e.status==='REQUEST_DENIED'||e.status==='OVER_QUERY_LIMIT')throw e;}
 }
 throw firstError||new Error('사용 가능한 경로를 찾지 못했습니다.');
}
function showApproximateRoute(a,b,name,error){
 clearRoutes();
 const path=[a,b];
 const line=new google.maps.Polyline({map,path,strokeColor:'#ff3b5c',strokeOpacity:.8,strokeWeight:5,icons:[{icon:{path:google.maps.SymbolPath.FORWARD_CLOSED_ARROW,scale:3,fillColor:'#fff',fillOpacity:1,strokeColor:'#ff3b5c'},offset:'55%'}]});
 routeLines.push(line);const bounds=new google.maps.LatLngBounds();path.forEach(x=>bounds.extend(x));map.fitBounds(bounds,70);
 const km=hav(a,b),mins=estimateMinutes(a,b,currentMode);
 $('#distanceMetric').textContent=`약 ${km.toFixed(1)} km`;$('#durationMetric').textContent=`약 ${Math.round(mins)}분`;
 $('#directionsPanel').innerHTML=`<div class="leg-card"><div class="leg-head"><b>내 위치 → ${esc(name)}</b><span>직선 기준 임시 안내</span></div><div class="step-list"><div class="step"><span>⚠️</span><span>${esc(error.message)}<br>실제 도로 경로를 사용하려면 Google Cloud에서 <b>Directions API</b>를 활성화해 주세요. 현재는 위치 관계를 확인할 수 있도록 직선으로 표시했습니다.</span><span></span></div></div></div>`;
}
...
async function drawRoute(){
 ...
 for(let i=0;i<stops.length-1;i++){try{
  ...
  const fallback=routed.fallback?' · 대중교통 없음 → 도보 연결':'';
